  ...
  try {
    ...
    try {
      if (dirHandle.name !== 'Clippings') {
        ...
        clippingsDir = await dirHandle.getDirectoryHandle('Clippings', { create: true });
      }
    } catch (innerErr) {
    ...
    await chrome.storage.local.set({ vaultClippingsChosen: true, vaultClippingsName: clippingsDir.name });
